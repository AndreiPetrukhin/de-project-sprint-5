...
def sprint5_example_stg_api_couriers_dag():
    ...
    def extract_couriers_data():
        ...
        while True:
            endpoint = f"couriers?sort_field=id&sort_direction=asc&limit={limit}&offset={page * limit}"
            couriers_data = api_utils.extract_data(endpoint=endpoint)
            if not couriers_data:
                break  # No more data

            filtered_data = [courier for courier in couriers_data if courier["_id"] > last_uploaded_id] if last_uploaded_id else couriers_data
            all_couriers_data.extend([(courier["_id"], courier) for courier in filtered_data])

            if len(couriers_data) < limit:
                break  # Last page

            page += 1

        return all_couriers_data
    ...
    def load_couriers_data_and_update_last_id(couriers_data):
        if couriers_data:
            ...
            api_utils.load_data_and_update_last_id(data=couriers_data, table_name="stg.api_couriers", columns=["courier_id", "object_value"], workflow_key=workflow_key, last_id=last_id)
