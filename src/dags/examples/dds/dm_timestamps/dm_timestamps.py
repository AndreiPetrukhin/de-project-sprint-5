...
@dag(
    ...
    catchup=False,
    tags=['sprint5', 'dds', 'timestamps'],
    is_paused_upon_creation=False
)
def sprint5_example_dds_dm_timestamps_dag():
        ...
        with PostgresHook(postgres_conn_id=dwh_pg_connect).get_conn() as conn:
            with conn.cursor() as cursor:
                cursor.execute("""
                    ...
                    WHERE workflow_key = 'stg_to_dds_timestamps'
                """)
            ...
            with conn.cursor() as cursor:
                try:
                    cursor.execute("""
                        ...
                        SELECT DISTINCT
                            ((object_value::jsonb)->>'date')::timestamp AS ts
                        FROM stg.ordersystem_orders
                        WHERE ((object_value::jsonb)->>'date')::timestamp > %s AND
                            (object_value::jsonb)->>'final_status'::text IN ('CLOSED', 'CANCELLED')
                        ON CONFLICT (ts) DO NOTHING
                    """, (last_loaded_id,))

                    cursor.execute("""
                        SELECT MAX(((object_value::jsonb)->>'date')::timestamp) FROM stg.ordersystem_orders
                        WHERE ((object_value::jsonb)->>'date')::timestamp > %s
                            AND (object_value::jsonb)->>'final_status'::text IN ('CLOSED', 'CANCELLED')
                    """, (last_loaded_id,))
                    ...
                    if new_last_loaded_id:
                        cursor.execute("""
                            ...
                            VALUES ('stg_to_dds_timestamps', jsonb_build_object('last_loaded_id', %s))
...
sprint5_example_dds_timestamps_dag = sprint5_example_dds_dm_timestamps_dag()
